...
let bodyTitle: HTMLHeadingElement;
let bodyContent: HTMLParagraphElement;

let footerContainer: HTMLElement;
let cancelBtn: HTMLButtonElement;
let okBtn: HTMLButtonElement;

let success: HTMLElement, info: HTMLElement, warning: HTMLElement, danger: HTMLElement;
...
function awake(): void {
    ...
    dismissButton = createElement('inline-flex flex-shrink-0 justify-center items-center h-8 w-8 rounded-md text-gray-500 hover:text-gray-400 focus:outline-none focus:ring-2 focus:ring-gray-400 focus:ring-offset-2 focus:ring-offset-white transition-all text-sm dark:focus:ring-gray-700 dark:focus:ring-offset-gray-800', 'button') as HTMLButtonElement;
    ...
    bodyTitle = createElement('mb-2 text-2xl font-bold text-gray-800 dark:text-gray-200', 'h3') as HTMLHeadingElement;
    bodyContent = createElement('text-gray-500', 'p') as HTMLParagraphElement;

    footerContainer = createElement('flex items-center');
    cancelBtn = createElement('p-4 w-full font-bold inline-flex justify-center items-center gap-2 rounded-bl-xl bg-gray-100 border border-transparent font-semibold text-gray-800 hover:text-blue-600 focus:outline-none focus:ring-2 ring-offset-white focus:ring-gray-100 focus:ring-offset-2 transition-all text-sm dark:bg-gray-700 dark:hover:bg-gray-600 dark:focus:ring-gray-600 dark:text-white dark:focus:ring-offset-gray-800', 'button') as HTMLButtonElement;
    okBtn = createElement('', 'button') as HTMLButtonElement;
    ...
    function makeAlert(color: string, icon: string): HTMLElement {
        const element = alert.cloneNode() as HTMLElement;
...
function createElement(className: string, tag: string = 'div'): HTMLElement|HTMLButtonElement|HTMLParagraphElement|HTMLHeadingElement {
...
export default function dialog(config: DialogConfig) {
    ...
    function close(triggerEvent: boolean = true) {
        ...
        if (triggerEvent && typeof config.onCancel === 'function') {
            config.onCancel();
        }
    }
