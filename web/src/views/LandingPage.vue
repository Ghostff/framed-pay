<template>
  <!-- Header Section -->
    <header class="relative z-50 w-full h-24">
        <div class="container flex items-center justify-center h-full max-w-6xl px-8 mx-auto sm:justify-between xl:px-0">
            ...
            <nav
                id="nav"
                class="absolute top-0 left-0 z-50 flex flex-col items-center justify-between hidden w-full h-64 pt-5 mt-24 text-sm bg-white border-t border-gray-200 md:w-auto md:flex-row md:h-24 lg:text-base md:bg-transparent md:mt-0 md:border-none md:py-0 md:flex md:relative"
            >
                ...
                <div class="flex flex-col block w-full font-medium border-t border-gray-200 md:hidden">
                    <a href="#" @click.prevent.stop="showAuthModal = true" class="w-full py-2 font-bold text-center text-pink-500">Login1</a>
  ...
  <!-- End Header Section-->
    ...
    <BasicModal :is-open="showAuthModal !== null" size="lg" @on:close="showAuthModal = null">
        <AuthenticationIndex :key="showAuthModal" :show="showAuthModal" />
    </BasicModal>

    <Main @on:signup="showAuthModal = 'register'" />
...
import AuthenticationIndex from "@/views/authentication/AuthenticationIndex.vue";
import { type RouteLocationNormalizedLoaded, useRoute} from "vue-router";
import Logo from "@/components/Logo.vue";
const showAuthModal: Ref = ref<string|null>(null);
