...
// https://preline.co/plugins/html/modal.html
onMounted(() => {
    toggle(props.isOpen);
    // @ts-ignore
    HSOverlay.on('open', (element: HTMLElement) => emit('on:open', element));
    // @ts-ignore
    HSOverlay.on('close', (element: HTMLElement) => emit('on:close', element));
});
